...
// Question1- randomly generated name, occupation, 
// and rate according to the provided constants.

function getFreelancer(){
    const name = NAMES[Math.floor(Math.random()* NAMES.length)];
    const occupation = OCCUPATIONS[Math.floor(Math.random()*OCCUPATIONS)];
...
function freelancerRow(freelancer){
 ...
 $rate.textContent = `$${freelancers.rate}`;
...
function freelancerRows(){
    ...
    for(const freelancer of freelancers){
        $tbody.id.append(freelancerRow(freelancer));
    }
